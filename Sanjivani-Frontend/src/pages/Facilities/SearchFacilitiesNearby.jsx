...
const SearchFacilitiesNearby = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true)
      try {
        const response = await axios.post('http://localhost:5001/get-hospital', { prompt: searchQuery });
  ...
  const loadRoute = (hospital) => {
    ...
    const routeUrl = `http://localhost:5001/route?startLat=${lat}&startLng=${lng}&endLat=${coordinates[1]}&endLng=${coordinates[0]}`;
