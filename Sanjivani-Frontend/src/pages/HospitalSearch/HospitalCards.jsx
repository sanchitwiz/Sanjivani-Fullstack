...
const HospitalCard = ({ hospital, isLoggedIn, adminSharedEmail }) => {
  ...
  useEffect(() => {
    socket.on('ambulanceAssigned', (data) => {
      ...
      setBookingDetails(prev => ({
        ...prev,
        trackingLink: `http://localhost:5001/ambulance/${data.ambulanceId}/track`,
      }));
    });
  ...
  const bookAmbulance = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5001/ambulance/client/request', {
        name: clientName,
